...
Future<void> PostEmailVerification(String email, BuildContext context) async {
  ...
  final url = Uri.parse("https://totaltravel.somee.com/API/Login/EmailVerification");
...
Future<void> PostEmailSender(String email, BuildContext context, int? id) async{
...
Future<void> verificationCode(int usercode, int? code, BuildContext context, int? id) async{
...
Future<void> changePassword(int? id, String password, BuildContext context) async {

 UserRegisterModel loginData = new UserRegisterModel();
  loginData.Usua_ID = id;
  loginData.Usua_Password = password;

  final url = Uri.parse("https://totaltravel.somee.com/API/Users/UpdatePassword");
  final headers = {"Content-type": "application/json", "Accept": "text/plain"};
  final json = jsonEncode(loginData);
